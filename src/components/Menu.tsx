...
import { useClerk } from "@clerk/nextjs"; // Add this import
          ...
          {section.items.map((item) =>
            item.label === "Logout" ? (
              <button
                key={item.href}
                onClick={() => signOut()}
                className="flex items-center justify-center lg:justify-start gap-4 py-2 text-black/90 hover:text-primary transition-colors"
                type="button"
              >
              ...
              <Link
                ...
                className={`flex items-center justify-center lg:justify-start gap-4 py-2 ${
                  pathname === item.href
                    ? "text-primary" // Change text color when active
                    : "text-black/90"
                }`}
              >
                <div className="flex items-center gap-2">
